...
export async function GET(request, { params }) {
  ...
  const platformRes = await fetch(`${platformUrl}/api/v1/mahalaxmi/servers/${id}`, {
  ...
  if (!platformRes.ok) {
    ...
    console.error(`[servers/${id}] platform error ${platformRes.status}`, errorBody);
    return Response.json({ error: 'platform_error', detail: errorBody }, { status: platformRes.status === 404 ? 404 : 502 });
  }
