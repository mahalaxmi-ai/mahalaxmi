...
export async function GET(request, { params }) {
  ...
  const platformRes = await fetch(`${platformUrl}/api/v1/mahalaxmi/servers/${id}/vscode-config`, {
  ...
  if (!platformRes.ok) {
    ...
    console.error(`[servers/${id}/vscode-config] platform error ${platformRes.status}`, errorBody);
